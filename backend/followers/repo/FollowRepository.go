...
// vraća broj korisnika koje dati korisnik prati
func (repo *FollowerRepository) GetFollowingCount(userID uuid.UUID) (int, error) {
	ctx := context.Background()
	session := repo.driver.NewSession(ctx, neo4j.SessionConfig{DatabaseName: "neo4j"})
	defer session.Close(ctx)

	countResult, err := session.ExecuteRead(ctx,
		func(transaction neo4j.ManagedTransaction) (any, error) {
			result, err := transaction.Run(ctx,
				`MATCH (user:User {id: $userId})-[:FOLLOWS]->(:User)
			...
			if result.Next(ctx) {
				record := result.Record()
				if countValue, ok := record.Get("count"); ok {
					return int(countValue.(int64)), nil
				}
			}
			return 0, result.Err()
		})
	...
	if err != nil {
		repo.logger.Printf("Error querying following count: %v", err)
		return 0, err
	}
	return countResult.(int), nil
}

// vraća broj pratilaca datog korisnika
func (repo *FollowerRepository) GetFollowersCount(userID uuid.UUID) (int, error) {
	ctx := context.Background()
	session := repo.driver.NewSession(ctx, neo4j.SessionConfig{DatabaseName: "neo4j"})
	defer session.Close(ctx)
	...
	countResult, err := session.ExecuteRead(ctx,
		func(transaction neo4j.ManagedTransaction) (any, error) {
			result, err := transaction.Run(ctx,
				`MATCH (:User)-[:FOLLOWS]->(user:User {id: $userId})
				 RETURN COUNT(*) as count`,
				map[string]any{"userId": userID.String()})
			if err != nil {
				return 0, err
	...
	if err != nil {
		repo.logger.Printf("Error querying followers count: %v", err)
		return 0, err
	}
	return countResult.(int), nil
}
